...
import '../tools/infovaluestring.dart';
import '../tools/paddedelevatedbutton.dart';
import 'five.dart';
...
class TwentySix extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: set.initText('TwentySix Page'),
      ),
      body: const SingleChildScrollView(
        child: TwentySixStateFul(),
      )
    );
  }
}

class TwentySixStateFul extends StatefulWidget {
  const TwentySixStateFul({Key? key, this.notificationAppLaunchDetails}) : super(key: key);
  ...
  @override
  State<TwentySixStateFul> createState() => TwentySixState();
}

class TwentySixState extends State<TwentySixStateFul> with WidgetsBindingObserver {
  ...
  void didChangeDependencies() {
    // 依賴變化
    if (kDebugMode) {
      print('TwentySix didChangeDependencies');
    }
    super.didChangeDependencies();
  }

  @override
  void didUpdateWidget(covariant TwentySixStateFul oldWidget) {
    // 元件發生變化
    if (kDebugMode) {
      print('TwentySix didUpdateWidget');
    }
            ...
            child: Center(
              child: Column(
                children: <Widget>[
                  ...
                  PaddedElevatedButton(
                    buttonText:
                    '您當地的時區安排每年星期一上午 10:00:00 通知',
                    onPressed: () async {
                      await _scheduleYearlyMondayTenAMNotification();
                    },
                  ),
                  PaddedElevatedButton(
                    buttonText: '顯示沒有聲音的通知',
                    onPressed: () async {
                  ...
                  if (!kIsWeb && Platform.isAndroid) ...<Widget>[
                    ...
                    PaddedElevatedButton(
                      buttonText:
                      '顯示帶有自定義振動模式的通知，紅色 LED 和紅色圖標',
                      onPressed: () async {
                        await _showNotificationCustomVibrationIconLed();
                      },
                    ),
                    PaddedElevatedButton(
                      buttonText: '使用 Android Uri 聲音顯示通知',
                      onPressed: () async {
                  ...
                  if (!kIsWeb &&
                    ...
                    const Text('iOS 和 macOS 特定示例',
                      style: TextStyle(fontWeight: FontWeight.bold),
                    ),
                    PaddedElevatedButton(
                      buttonText: '顯示帶有字幕的通知',
                      onPressed: () async {
                        await _showNotificationWithSubtitle();
                      },
                    ),
                    PaddedElevatedButton(
                      buttonText: '顯示帶有圖標徽章的通知',
                      onPressed: () async {
                        await _showNotificationWithIconBadge();
                      },
                    ),
                    PaddedElevatedButton(
                      buttonText: '顯示帶有附件的通知',
                      onPressed: () async {
                        await _showNotificationWithAttachment();
                      },
                    ),
                    PaddedElevatedButton(
                      buttonText: '顯示帶有線程標識符的通知',
                      onPressed: () async {
  ...
  Future<void> _showNotificationCustomVibrationIconLed() async {
    ...
    NotificationDetails(android: androidPlatformChannelSpecifics);
    await flutterLocalNotificationsPlugin.show(0, 'title of notification with custom vibration pattern, LED and icon', 'body of notification with custom vibration pattern, LED and icon', platformChannelSpecifics);
  }
...
Future<void> _showLinuxNotificationWithByteDataIcon() async {
  ...
  final NotificationDetails platformChannelSpecifics = NotificationDetails(linux: linuxPlatformChannelSpecifics,);
  await flutterLocalNotificationsPlugin.show(0, 'notification with byte data icon', null, platformChannelSpecifics,);
}
...
Future<void> _showLinuxNotificationWithThemeIcon() async {
  ...
  final NotificationDetails platformChannelSpecifics = NotificationDetails(linux: linuxPlatformChannelSpecifics,);
  await flutterLocalNotificationsPlugin.show(0, 'notification with theme icon', null, platformChannelSpecifics,);
}
...
Future<void> _showLinuxNotificationWithThemeSound() async {
  ...
  final NotificationDetails platformChannelSpecifics = NotificationDetails(linux: linuxPlatformChannelSpecifics,);
  await flutterLocalNotificationsPlugin.show(0, 'notification with theme sound', null, platformChannelSpecifics,);
}
...
Future<void> _showLinuxNotificationWithCriticalUrgency() async {
  ...
  await flutterLocalNotificationsPlugin.show(0, 'notification with critical urgency', null, platformChannelSpecifics,);
}

Future<void> _showLinuxNotificationWithTimeout() async {
  final LinuxNotificationDetails linuxPlatformChannelSpecifics = LinuxNotificationDetails(timeout: LinuxNotificationTimeout.fromDuration(const Duration(seconds: 1),),);
  final NotificationDetails platformChannelSpecifics = NotificationDetails(linux: linuxPlatformChannelSpecifics,);
  await flutterLocalNotificationsPlugin.show(0, 'notification with timeout', null, platformChannelSpecifics,);
}

Future<void> _showLinuxNotificationSuppressSound() async {
  const LinuxNotificationDetails linuxPlatformChannelSpecifics = LinuxNotificationDetails(suppressSound: true,);
  const NotificationDetails platformChannelSpecifics = NotificationDetails(linux: linuxPlatformChannelSpecifics,);
  await flutterLocalNotificationsPlugin.show(0, 'suppress notification sound', null, platformChannelSpecifics,);
}

Future<void> _showLinuxNotificationTransient() async {
  const LinuxNotificationDetails linuxPlatformChannelSpecifics = LinuxNotificationDetails(transient: true,);
  const NotificationDetails platformChannelSpecifics = NotificationDetails(linux: linuxPlatformChannelSpecifics,);
  await flutterLocalNotificationsPlugin.show(0, 'transient notification', null, platformChannelSpecifics,);
}
...
Future<void> _showLinuxNotificationResident() async {
  ...
  const NotificationDetails platformChannelSpecifics = NotificationDetails(linux: linuxPlatformChannelSpecifics,);
  await flutterLocalNotificationsPlugin.show(0, 'resident notification', null, platformChannelSpecifics,);
}
...
Future<void> _showLinuxNotificationDifferentLocation() async {
  ...
  const NotificationDetails platformChannelSpecifics = NotificationDetails(linux: linuxPlatformChannelSpecifics,);
  await flutterLocalNotificationsPlugin.show(0, 'notification on different screen location', null, platformChannelSpecifics,);
}
