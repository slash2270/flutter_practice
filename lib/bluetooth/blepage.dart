...
import 'package:flutter_practice/bluetooth/sputil.dart';
...
class BlePageState extends State<BlePage> implements DeviceListener {
  ...
  void initState() {
    ...
    Timer(Duration(milliseconds: 200), () {
      ...
      Fluttertoast.showToast(msg: S.of(context).ble_connecting, gravity: ToastGravity.CENTER);
    });
    //监听输入框焦点得失
                ...
                children: [
                  ...
                  Ink(
                    child: InkWell(
                      onTap: () {
                        if (!isBLEConnected) {
                          Fluttertoast.showToast(msg: S.of(context).ble_disconnet_connecting, gravity: ToastGravity.CENTER);
                          Ble.getInstance().connect(widget.deviceBle.address);
                          Timer(Duration(milliseconds: 10000), () {
                            Ble.getInstance().disconnect();
                            Fluttertoast.showToast(msg: S.of(context).connect_time_out_check_whether_ble_is_turned_on, gravity: ToastGravity.CENTER);
                          });
                          return;
                        }
                        // 发送的指令不能为空
                        if (sendCommand.isEmpty) {
                          Fluttertoast.showToast(msg: S.of(context).please_input_order, gravity: ToastGravity.CENTER);
                          return;
                        }
